...
export async function GET(req: NextRequest) {
  ...
  const data = await prisma.socialLogin.findMany({ orderBy: { name: "asc" } });
...
export async function PATCH(req: NextRequest) {
  ...
  const existing = await prisma.socialLogin.findUnique({ where: { id } });
  if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });

  const item = await prisma.socialLogin.update({ where: { id }, data: body });
