...
export async function GET(req: NextRequest) {
  ...
  const data = await prisma.menuTemplate.findMany({ orderBy: { name: "asc" } });
...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.menuTemplate.create({ data: body });
...
export async function PATCH(req: NextRequest) {
  ...
  const item = await prisma.menuTemplate.update({ where: { id }, data: body });
