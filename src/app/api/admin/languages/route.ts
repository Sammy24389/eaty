...
export async function GET(req: NextRequest) {
  ...
  const data = await prisma.language.findMany({ orderBy: { name: "asc" } });
...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.language.create({ data: body });
...
export async function PATCH(req: NextRequest) {
  ...
  const item = await prisma.language.update({ where: { id }, data: body });
  return NextResponse.json({ data: item });
}
