...
export async function GET(req: NextRequest) {
  ...
  const id = getId(req);
  if (id) {
    const item = await prisma.timeSlot.findUnique({ where: { id } });
...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.timeSlot.create({ data: body });
...
export async function PATCH(req: NextRequest) {
  ...
  const item = await prisma.timeSlot.update({ where: { id }, data: body });
  return NextResponse.json({ data: item });
}
