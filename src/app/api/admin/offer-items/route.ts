...
export async function GET(req: NextRequest) {
  ...
  if (offerId) where.offerId = BigInt(offerId);

  const data = await prisma.offerItem.findMany({ where, include: { offer: true, item: true } });
...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.offerItem.create({ data: body });
...
export async function DELETE(req: NextRequest) {
  ...
  const existing = await prisma.offerItem.findUnique({ where: { id } });
