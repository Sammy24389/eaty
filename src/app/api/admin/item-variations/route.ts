...
export async function GET(req: NextRequest) {
  ...
  const data = await prisma.itemVariation.findMany({ where, include: { item: true, attribute: true } });
...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.itemVariation.create({ data: body });
...
export async function PATCH(req: NextRequest) {
  ...
  const body = await req.json();
  const existing = await prisma.itemVariation.findUnique({ where: { id } });
  if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });

  const item = await prisma.itemVariation.update({ where: { id }, data: body });
...
export async function DELETE(req: NextRequest) {
  ...
  await prisma.itemVariation.delete({ where: { id } });
