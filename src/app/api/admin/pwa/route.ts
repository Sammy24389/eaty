...
export async function GET(req: NextRequest) {
  ...
  const data = await prisma.pWA.findMany();
...
export async function POST(req: NextRequest) {
  ...
  const existing = await prisma.pWA.findFirst();

  let item;
  if (existing) {
    item = await prisma.pWA.update({ where: { id: existing.id }, data: body });
  } else {
    item = await prisma.pWA.create({ data: body });
  }
