...
export async function GET(req: NextRequest) {
  ...
  const id = getId(req);
  if (id) {
    const item = await prisma.itemCategory.findUnique({ where: { id } });
  ...
  const [data, total] = await Promise.all([
    prisma.itemCategory.findMany({ where, skip: (page - 1) * limit2, take: limit2, orderBy: { sort: "asc" } }),
...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.itemCategory.create({ data: body });
...
export async function PATCH(req: NextRequest) {
  ...
  const item = await prisma.itemCategory.update({ where: { id }, data: body });
  return NextResponse.json({ data: item });
}
