...
export async function GET(req: NextRequest) {
  ...
  const data = await prisma.notificationAlert.findMany({ orderBy: { name: "asc" } });
...
export async function PATCH(req: NextRequest) {
  ...
  const existing = await prisma.notificationAlert.findUnique({ where: { id } });
  if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });

  const item = await prisma.notificationAlert.update({ where: { id }, data: body });
