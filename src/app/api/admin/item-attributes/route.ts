...
export async function GET(req: NextRequest) {
  ...
  const data = await prisma.itemAttribute.findMany({ orderBy: { name: "asc" } });
...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.itemAttribute.create({ data: body });
...
export async function PATCH(req: NextRequest) {
  ...
  const body = await req.json();
  const existing = await prisma.itemAttribute.findUnique({ where: { id } });
  if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });

  const item = await prisma.itemAttribute.update({ where: { id }, data: body });
