...
export async function GET(req: NextRequest) {
  ...
  const id = getId(req);
  if (id) {
    const item = await prisma.tax.findUnique({ where: { id } });
  ...
  const [data, total] = await Promise.all([
    prisma.tax.findMany({ where, skip: (page - 1) * limit2, take: limit2 }),
...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.tax.create({ data: body });
...
export async function PATCH(req: NextRequest) {
  ...
  const item = await prisma.tax.update({ where: { id }, data: body });
  return NextResponse.json({ data: item });
}
