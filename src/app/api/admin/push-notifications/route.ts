...
export async function GET(req: NextRequest) {
  ...
  const id = getId(req);
  if (id) {
    const item = await prisma.pushNotification.findUnique({ where: { id } });
  ...
  const [data, total] = await Promise.all([
    prisma.pushNotification.findMany({ skip: (page - 1) * limit2, take: limit2, orderBy: { createdAt: "desc" } }),
...
export async function POST(req: NextRequest) {
  ...
  const item = await prisma.pushNotification.create({ data: body });
...
export async function DELETE(req: NextRequest) {
  ...
  const existing = await prisma.pushNotification.findUnique({ where: { id } });
